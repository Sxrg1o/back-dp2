...
run_test() {
    ...
    if [ "$status_code" = "$expected_status" ]; then
        echo -e "$✓ PASS${NC} (Status: $status_code)"
        PASSED_TESTS=$((PASSED_TESTS + 1))
        echo "$body"
        return 0
    else
        echo -e "$✗ FAIL${NC} (Expected: $expected_status, Got: $status_code)"
...
if [ -n "$MESA_ID" ]; then
    echo -e "$✓${NC} Mesa ID: $MESA_ID"
else
    echo -e "$✗ No se encontraron mesas${NC}"
    exit 1
fi
...
if [ -n "$PRODUCTO_ID" ]; then
    echo -e "$✓${NC} Producto: $PRODUCTO_NOMBRE (ID: $PRODUCTO_ID, Precio: S/$PRODUCTO_PRECIO)"
else
    echo -e "$✗ No se encontraron productos${NC}"
    exit 1
fi
...
if [ -n "$PEDIDO_ID" ] && [ ${#PEDIDO_ID} -eq 26 ]; then
    echo -e "$✓ PASS${NC} (ID: $PEDIDO_ID)"
    PASSED_TESTS=$((PASSED_TESTS + 1))
else
    echo -e "$✗ FAIL${NC} (ID inválido o vacío)"
    FAILED_TESTS=$((FAILED_TESTS + 1))
fi
...
if [ -n "$NUMERO_PEDIDO" ]; then
    echo -e "$✓ PASS${NC} (Número: $NUMERO_PEDIDO)"
    PASSED_TESTS=$((PASSED_TESTS + 1))
else
    echo -e "$✗ FAIL${NC} (Número de pedido vacío)"
    FAILED_TESTS=$((FAILED_TESTS + 1))
fi
...
if [ "$ESTADO" = "pendiente" ]; then
    echo -e "$✓ PASS${NC} (Estado: $ESTADO)"
    PASSED_TESTS=$((PASSED_TESTS + 1))
else
    echo -e "$✗ FAIL${NC} (Estado esperado: pendiente, obtenido: $ESTADO)"
    FAILED_TESTS=$((FAILED_TESTS + 1))
fi
...
if [ "$SUBTOTAL" = "$PRODUCTO_PRECIO" ]; then
    echo -e "$✓ PASS${NC} (Subtotal: S/$SUBTOTAL)"
    PASSED_TESTS=$((PASSED_TESTS + 1))
else
    echo -e "$✗ FAIL${NC} (Esperado: S/$PRODUCTO_PRECIO, Obtenido: S/$SUBTOTAL)"
    FAILED_TESTS=$((FAILED_TESTS + 1))
fi
...
if [ "$TOTAL" = "$SUBTOTAL" ]; then
    echo -e "$✓ PASS${NC} (Total: S/$TOTAL)"
    PASSED_TESTS=$((PASSED_TESTS + 1))
else
    echo -e "$✗ FAIL${NC} (Total: S/$TOTAL != Subtotal: S/$SUBTOTAL)"
    FAILED_TESTS=$((FAILED_TESTS + 1))
fi
...
if [ "$ITEMS_COUNT" = "1" ]; then
    echo -e "$✓ PASS${NC} (Items: $ITEMS_COUNT)"
    PASSED_TESTS=$((PASSED_TESTS + 1))
else
    echo -e "$✗ FAIL${NC} (Esperado: 1 item, Obtenido: $ITEMS_COUNT)"
    FAILED_TESTS=$((FAILED_TESTS + 1))
fi
...
if [ -n "$ITEM_ID" ] && [ "$ITEM_CANTIDAD" = "1" ] && [ "$ITEM_PRECIO_OPCIONES" = "0.0" ]; then
    echo -e "$✓ PASS${NC} (ID: OK, Cantidad: $ITEM_CANTIDAD, Precio opciones: S/$ITEM_PRECIO_OPCIONES)"
    PASSED_TESTS=$((PASSED_TESTS + 1))
else
    echo -e "$✗ FAIL${NC} (Estructura incompleta o incorrecta)"
    FAILED_TESTS=$((FAILED_TESTS + 1))
fi
...
if [ -n "$PEDIDO_ID" ]; then
    ...
    echo -e "$⚠ SKIP${NC} - No se pudo crear pedido, tests de consulta omitidos"
fi
...
echo -e "Pasados: $$PASSED_TESTS${NC}"
echo -e "Fallidos: $$FAILED_TESTS${NC}"
echo ""

if [ $FAILED_TESTS -eq 0 ]; then
    echo -e "$✓ Todos los tests pasaron${NC}"
    exit 0
else
    echo -e "$✗ Algunos tests fallaron${NC}"
    exit 1
fi
