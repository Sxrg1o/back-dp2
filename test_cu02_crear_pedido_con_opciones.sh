...
PRODUCTO_RESPONSE=$(curl -s "$API_URL/api/v1/productos/cards?limit=50")

# CORRECCION: Buscar producto que tenga opciones
PRODUCTO_ID=$(echo "$PRODUCTO_RESPONSE" | py -c "
...
for item in data.get('items', []):
    ...
    if producto_id:
        ...
        try:
            ...
            producto_data = json.loads(response.read())
            if producto_data.get('tipos_opciones'):
                print(producto_id)
                break
        except:
            continue
...
if [ -z "$PRODUCTO_ID" ]; then
    echo -e "${RED}✗ No se encontraron productos con opciones${NC}"
    exit 1
fi
...
# CORRECCION: python3 -> py
PRODUCTO_PRECIO=$(echo "$PRODUCTO_OPC_RESPONSE" | py -c "import sys, json; data = json.load(sys.stdin); print(data.get('precio_base', 0))" 2>/dev/null)
PRODUCTO_NOMBRE=$(echo "$PRODUCTO_OPC_RESPONSE" | py -c "import sys, json; data = json.load(sys.stdin); print(data.get('nombre', ''))" 2>/dev/null)

# Obtener IDs de opciones (hasta 2)
# CORRECCION: python3 -> py
# CORRECCION: Buscar en tipos_opciones (estructura anidada)
OPCION_IDS=$(echo "$PRODUCTO_OPC_RESPONSE" | py -c "
import sys, json
data = json.load(sys.stdin)
tipos_opciones = data.get('tipos_opciones', [])
count = 0
for tipo in tipos_opciones:
    for opc in tipo.get('opciones', []):
        if count < 2:
...
# CORRECCION: Logs a stderr, solo JSON capturado en variable
...
PRECIO_OPC_ESPERADO=$(py -c "print($OPCION1_PRECIO + $OPCION2_PRECIO)")

# Comparar como números flotantes
PRECIO_MATCH=$(py -c "print(float('$ITEM_PRECIO_OPCIONES') == float('$PRECIO_OPC_ESPERADO'))")
if [ "$PRECIO_MATCH" = "True" ]; then
...
# CORRECCION: python3 -> py
OPCIONES_COUNT=$(echo "$PEDIDO_RESPONSE" | py -c "import sys, json; data = json.load(sys.stdin); print(len(data['items'][0].get('opciones', [])) if data.get('items') else 0)" 2>/dev/null)
...
SUBTOTAL_ESPERADO=$(py -c "print(2 * ($PRODUCTO_PRECIO + $PRECIO_OPC_ESPERADO))")

# Comparar como números flotantes
SUBTOTAL_MATCH=$(py -c "print(float('$ITEM_SUBTOTAL') == float('$SUBTOTAL_ESPERADO'))")
if [ "$SUBTOTAL_MATCH" = "True" ]; then
...
# Comparar como números flotantes
PEDIDO_SUBTOTAL_MATCH=$(py -c "print(float('$PEDIDO_SUBTOTAL') == float('$SUBTOTAL_ESPERADO'))")
if [ "$PEDIDO_SUBTOTAL_MATCH" = "True" ]; then
...
# CORRECCION: python3 -> py
OPCION1_TIENE_ID=$(echo "$PEDIDO_RESPONSE" | py -c "import sys, json; data = json.load(sys.stdin); print('id' in data['items'][0]['opciones'][0] if data.get('items') and data['items'][0].get('opciones') else False)" 2>/dev/null)
OPCION1_TIENE_PRECIO=$(echo "$PEDIDO_RESPONSE" | py -c "import sys, json; data = json.load(sys.stdin); print('precio_adicional' in data['items'][0]['opciones'][0] if data.get('items') and data['items'][0].get('opciones') else False)" 2>/dev/null)
...
# Item 2: 2 * (precio + 0)
SUBTOTAL_MULTI_ESPERADO=$(py -c "print(1 * ($PRODUCTO_PRECIO + $OPCION1_PRECIO) + 2 * $PRODUCTO_PRECIO)")

# Comparar como números flotantes
if [ "$SUBTOTAL_MULTI" = "-1" ]; then
    echo -e "${RED}✗ FAIL${NC} (Error al obtener subtotal - pedido no se creó correctamente)"
    FAILED_TESTS=$((FAILED_TESTS + 1))
else
    SUBTOTAL_MULTI_MATCH=$(py -c "print(float('$SUBTOTAL_MULTI') == float('$SUBTOTAL_MULTI_ESPERADO'))")
    if [ "$SUBTOTAL_MULTI_MATCH" = "True" ]; then
