...
from src.api.schemas.producto_opcion_schema import ProductoOpcionCreate, ProductoOpcionUpdate
...
def producto_opcion_service(mock_repository):
    ...
    """
    service = ProductoOpcionService(AsyncMock())
...
@pytest.mark.asyncio
async def test_create_producto_opcion_success(producto_opcion_service, mock_repository, sample_producto_opcion_data):
    """
    ...
    PROCESO:
        ...
        - Llama al método create_producto_opcion con datos válidos.
    ...
    """
    # Arrange
    producto_opcion_create = ProductoOpcionCreate(
    ...
    mock_repository.create.return_value = ProductoOpcionModel(**sample_producto_opcion_data)

    # Act
    result = await producto_opcion_service.create_producto_opcion(producto_opcion_create)
    ...
    # Verificar que se pasó un objeto ProductoOpcionModel al repositorio
    args, _ = mock_repository.create.call_args
    assert isinstance(args[0], ProductoOpcionModel)
    assert args[0].nombre == sample_producto_opcion_data["nombre"]
    assert args[0].id_producto == sample_producto_opcion_data["id_producto"]


@pytest.mark.asyncio
async def test_create_producto_opcion_duplicate(producto_opcion_service, mock_repository, sample_producto_opcion_data):
    """
    ...
    PROCESO:
        ...
        - Llama al método create_producto_opcion con un nombre duplicado.
        - Verifica que se lance la excepción adecuada.

    POSTCONDICIONES:
        - El servicio debe lanzar ProductoOpcionConflictError.
    """
    # Arrange
    producto_opcion_create = ProductoOpcionCreate(
        id_producto=sample_producto_opcion_data["id_producto"],
    ...
    # Act & Assert
    with pytest.raises(ProductoOpcionConflictError) as excinfo:
        await producto_opcion_service.create_producto_opcion(producto_opcion_create)

    assert "Ya existe una opción de producto con el nombre" in str(excinfo.value)
    mock_repository.create.assert_called_once()


@pytest.mark.asyncio
async def test_get_producto_opcion_by_id_success(producto_opcion_service, mock_repository, sample_producto_opcion_data):
    """
    ...
    PROCESO:
        ...
        - Llama al método get_producto_opcion_by_id con un ID válido.
        - Verifica el resultado y las llamadas al mock.
    ...
    mock_repository.get_by_id.return_value = ProductoOpcionModel(**sample_producto_opcion_data)

    # Act
    result = await producto_opcion_service.get_producto_opcion_by_id(producto_opcion_id)
...
@pytest.mark.asyncio
async def test_get_producto_opcion_by_id_not_found(producto_opcion_service, mock_repository):
    """
    ...
    PROCESO:
        ...
        - Llama al método get_producto_opcion_by_id con un ID inexistente.
        - Verifica que se lance la excepción adecuada.
    ...
    # Act & Assert
    with pytest.raises(ProductoOpcionNotFoundError) as excinfo:
        await producto_opcion_service.get_producto_opcion_by_id(producto_opcion_id)

    assert f"No se encontró la opción de producto con ID {producto_opcion_id}" in str(excinfo.value)
    mock_repository.get_by_id.assert_called_once_with(producto_opcion_id)


@pytest.mark.asyncio
async def test_delete_producto_opcion_success(producto_opcion_service, mock_repository, sample_producto_opcion_data):
    """
    ...
    PROCESO:
        ...
        - Llama al método delete_producto_opcion con un ID válido.
        - Verifica el resultado y las llamadas al mock.
...
@pytest.mark.asyncio
async def test_delete_producto_opcion_not_found(producto_opcion_service, mock_repository):
    """
    ...
    PROCESO:
        - Configura el mock para simular que la opción de producto no existe.
        - Llama al método delete_producto_opcion con un ID inexistente.
        - Verifica que se lance la excepción adecuada.
    ...
    # Act & Assert
    with pytest.raises(ProductoOpcionNotFoundError) as excinfo:
        await producto_opcion_service.delete_producto_opcion(producto_opcion_id)
...
@pytest.mark.asyncio
async def test_get_producto_opciones_success(producto_opcion_service, mock_repository, sample_producto_opcion_data):
    """
    ...
    PROCESO:
        ...
        - Llama al método get_producto_opciones con parámetros válidos.
        - Verifica el resultado y las llamadas al mock.
    ...
    # Act
    result = await producto_opcion_service.get_producto_opciones(skip=0, limit=10)
...
@pytest.mark.asyncio
async def test_get_producto_opciones_validation_error(producto_opcion_service):
    """
    ...
    PROCESO:
        - Llama al método get_producto_opciones con parámetros inválidos.
        - Verifica que se lance la excepción adecuada.

    POSTCONDICIONES:
        - El servicio debe lanzar ProductoOpcionValidationError.
    """
    # Act & Assert - Parámetro skip negativo
    with pytest.raises(ProductoOpcionValidationError) as excinfo:
        await producto_opcion_service.get_producto_opciones(skip=-1, limit=10)
    assert "El parámetro 'skip' debe ser mayor o igual a cero" in str(excinfo.value)

    # Act & Assert - Parámetro limit no positivo
    with pytest.raises(ProductoOpcionValidationError) as excinfo:
        await producto_opcion_service.get_producto_opciones(skip=0, limit=0)
    assert "El parámetro 'limit' debe ser mayor a cero" in str(excinfo.value)


@pytest.mark.asyncio
async def test_update_producto_opcion_success(producto_opcion_service, mock_repository, sample_producto_opcion_data):
    """
    ...
    PROCESO:
        ...
        - Llama al método update_producto_opcion con datos válidos.
        - Verifica el resultado y las llamadas al mock.
    ...
    # Arrange
    producto_opcion_id = sample_producto_opcion_data["id"]
    update_data = ProductoOpcionUpdate(
        nombre="Ají picante",
        precio_adicional=Decimal("2.50")
    )

    updated_producto_opcion = ProductoOpcionModel(
        **{
    ...
    # Act
    result = await producto_opcion_service.update_producto_opcion(producto_opcion_id, update_data)
...
@pytest.mark.asyncio
async def test_update_producto_opcion_not_found(producto_opcion_service, mock_repository):
    """
    ...
    PROCESO:
        - Configura el mock para simular que la opción de producto no existe.
        - Llama al método update_producto_opcion con un ID inexistente.
        - Verifica que se lance la excepción adecuada.
    ...
    # Act & Assert
    with pytest.raises(ProductoOpcionNotFoundError) as excinfo:
        await producto_opcion_service.update_producto_opcion(producto_opcion_id, update_data)
...
@pytest.mark.asyncio
async def test_update_producto_opcion_duplicate_name(producto_opcion_service, mock_repository, sample_producto_opcion_data):
    """
    ...
    PROCESO:
        - Configura el mock para simular un error de integridad.
        - Llama al método update_producto_opcion con un nombre duplicado.
        - Verifica que se lance la excepción adecuada.
    ...
    # Arrange
    producto_opcion_id = sample_producto_opcion_data["id"]
    update_data = ProductoOpcionUpdate(nombre="Opción Duplicada")
...
@pytest.mark.asyncio
async def test_update_producto_opcion_no_changes(producto_opcion_service, mock_repository, sample_producto_opcion_data):
    """
    ...
    PROCESO:
        - Configura el mock para simular la existencia de la opción de producto.
        - Llama al método update_producto_opcion sin proporcionar campos para actualizar.
    ...
    # Arrange
    producto_opcion_id = sample_producto_opcion_data["id"]
    update_data = ProductoOpcionUpdate()  # Sin datos para actualizar
    mock_repository.get_by_id.return_value = ProductoOpcionModel(**sample_producto_opcion_data)
